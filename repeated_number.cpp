...
int main(){
    ...
    vector<long long> finalVec;

    long long int n = A.size();

   long long  sumN = (n*(n+1))/2,sumNSquare = (n*(n+1) * (2*n+1))/6;
    long long  sumArr =0,sumArrSquare=0;

    for (int i = 0; i < A.size(); i++)
    {
        sumArr+=A[i];
        sumArrSquare+=pow(A[i],2);
    }
    //  cout<<sumArr<<" "<<sumArrSquare;
    //(x-y)
   long long int xminusy = sumN-sumArr;
    // cout<<sumN-sumArr<<" ";
    //(x-y)^2
    //cout<<sumNSquare-sumArrSquare;
    //(x+y)(x-y)/(x-y) = (x+y)
   long long int xplusy = (sumNSquare-sumArrSquare)/xminusy;
   long long int missingNum = (xplusy+xminusy)/2;
   long long int repeatedNum = xplusy-missingNum;
    
    finalVec.push_back(repeatedNum);
    finalVec.push_back(missingNum);
